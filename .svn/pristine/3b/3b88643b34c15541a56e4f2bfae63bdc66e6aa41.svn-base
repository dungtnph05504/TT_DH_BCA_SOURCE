...
<form:form id="ricTxnUpldRptForm"
	modelAttribute="ricTxnUpldRptDto" method="POST" enctype="multipart/form-data"
...
<!-- 								<option value="nin">NIN</option>
								...
								<option value="txnUpldDate">Transaction Upload DateTime</option>
								<option value="txnStatus">Transaction Upload Status</option> -->
								</select></td>
							<td>
								<table>
									<tr>
							...
							<td><form:select id="selRptcols"
								...
								<option value="txnUpldStatus">Transaction Upload Status</option>								
							</form:select></td>
						</tr>
					</table>
				</td>
...
$(function() {  
	...
	$("#genBtnId").click(function(){ 
		 if($("#rptFrmt").val() == 'pdf'){
			...
			var orig = $("#ricTxnUpldRptForm").serialize();
			var withoutEmpties = orig.replace(/[^&]+=\.?(?:&|$)/g, '');
			var test = "${ricTxnUpldPdfResultUrl}" + '?' + withoutEmpties;
			document.forms["ricTxnUpldRptForm"].action = test;  
			document.forms["ricTxnUpldRptForm"].method = "POST";   
			//document.forms["ricTxnUpldRptForm"].onSubmit=window.showModalDialog(test,null, "dialogWidth:1280px;dialogHeight:800px;resizable:yes;addressbar:no;");
			
			/* document.forms["ricTxnUpldRptForm"].onSubmit = window.open(test, "popup", "location=no"); */
			window.open(test, "popup", "location=no,resizable=yes,left=0,top=0,height=700px,width=1280px");
				...
				var orig = $("#ricTxnUpldRptForm").serialize();
				var withoutEmpties = orig.replace(/[^&]+=\.?(?:&|$)/g, '');
				var test = "${showUnColCardStusExcelResult}" + '?' + withoutEmpties;
				document.forms["ricTxnUpldRptForm"].action = test;  
				document.forms["ricTxnUpldRptForm"].method = "POST";   
				//document.forms["ricTxnUpldRptForm"].onSubmit=window.showModalDialog(test,null, "dialogWidth:1280px;dialogHeight:800px;resizable:yes;addressbar:no;");
	...
	$('#selectAllCol2BtnId').click(function(){
	 ...
	 $("#resetBtn").click(function(){ 
		 	$("#crdRejStDt").val(""); 
			   ...
			   {'value':'txnSubType','text':'Transaction Sub Type'},
			   {'value':'applicationDate','text':'Transaction Date'},
			   {'value':'txnStatus','text':'Transaction Status'},
			   {'value':'txnUpldStatus','text':'Transaction Upload Status'}
			     ];  
			...
			columns.push(jsonObj); 
			 ...
			 $("#showHtmlRpt").empty();
				...
				$("#tranRptflexme").flexigrid({ 
					...
					sortorder : "asc",
					title : 'TRANSACTION UPLOAD REPORT',
					usepager : true,
					useRp : true,
					rp : 10,
