 ...
 */
@Repository("nicImmiHistoryImagesDao")
public class NicImmiHistoryImagesDaoImpl 
		extends GenericHibernateDao<ImmiHistoryImages, Long> 
		implements NicImmiHistoryImagesDao{
	
	@Transactional(rollbackFor = java.lang.Throwable.class, propagation = Propagation.SUPPORTS)
	@Override
	public List<ImmiHistoryImages> findByImmiId(Long id) {
 		DetachedCriteria detachedCriteria = DetachedCriteria.forClass(getPersistentClass());
 		detachedCriteria.add(Restrictions.eq("immihistoryId", id));
		List<ImmiHistoryImages> resultList = this.findAll(detachedCriteria);
		return resultList;
	}
