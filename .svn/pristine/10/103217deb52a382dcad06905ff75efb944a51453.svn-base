...
<c:url var="listAfterWFUrl" value="/servlet/workflowProcess/listAfterwf" />
...
<c:url var="changePageUrl" value="/servlet/workflowProcess/mainNo" />
					...
					<div class="stepwizard-step col-xs-3"> 
						<c:choose>
    						<c:when test="${checkCPD.status == 1}">
								<a id ="idCHECK_CPD" type="button" class="btn btn-success btn-circle" onclick="reprocess('CHECK_CPD')">Kiểm tra CPD <p>(Không lỗi)</p></a>
							</c:when>    
    						<c:otherwise>
    							<a id="idCHECK_CPD" type="button" class="btn btn-success btn-circle-unactive" onclick="reprocess('CHECK_CPD')">Kiểm tra CPD <p>(Không lỗi)</p></a>
						 	</c:otherwise>
						</c:choose>
...
<script>
	...
	function changeWflEnd(k, n){
		...
		$('#wflStart').html("");
		$('#wflStart').html("" + nameS); 
		$('#dialog-workflow-change-end').dialog('option', 'title', 'Thay đổi kết thúc của tiến trình ' + nameS);
		$.ajax({
			type : "GET",
			...
			success : function(data) {
				...
				$.ajax({
					...
					success : function(json) {
						...
						keyS = k;
						numS = n;
						$("#dialog-workflow-change-end").dialog("open");
					},
					error : function(e) {
