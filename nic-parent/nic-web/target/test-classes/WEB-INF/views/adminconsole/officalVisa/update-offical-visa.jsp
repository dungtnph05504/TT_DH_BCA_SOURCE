...
<c:url var="resetPage" value="/servlet/officeVisaController/updateOfficalVisa" />
...
function ClearDDL(){
  ...
  $('#add_btn').click(function() {
	  if(checkValid()){
		  ...
		  $.post('<c:url value="/servlet/paymentMatrixController/updateNationVisaA" />',stringUrl,
		             function(data){
			   if(data=='success'){
			   	   $("#savedialog-confirm").dialog('option', 'title', 'Thông báo');
			       $("#savedialog-confirm").html('Đã sửa thành công Quốc gia miễn thị thực mã :  '+$("#countryCode").val());
			       ...
			       $("#faildialog-confirm").dialog( 'open' );
				       ...
				       $("#faildialog-confirm").dialog('option', 'title', 'Thông báo');
				       $("#faildialog-confirm").html('Không thể sửa Quốc gia miễn thị thực mã:  '+$("#countryCode").val());
				       $("#faildialog-confirm").dialog( 'open' );
				}
		...
		<div class="container">
			<div class="row">
				<div class="roundedBorder ov_hidden">
					<div class="new-heading-2">CẬP NHẬT THÔNG TIN QUỐC GIA MIỄN THỊ THỰC</div>
					&nbsp;
					<div id="update_msg" style="display: none;">
						<!-- <img src="images/info1.jpg" width="30" height="30"> -->&nbsp;
						<span style="font-size: 14px; color: #0000FF">Quốc gia miễn thị thực đã được lưu thành công
					</div>
					...
					<form:form modelAttribute="officalVisaForm"
						...
						name="officalVisaForm" class="inline">
						<div class="col-sm-6">
							...
							<div class="form-group">
								...
								<form:input class="form-control" type="text" path="freeday" id="freeday"/>
							</div>
							<div class="form-group">
					...
					<table style="text-align: right;">
						<tr>
							<td colspan="2" align="right" style="padding: 10px; text-align: right;">
								...
								<input type="button" class="btn btn-primary" id="resetBtn" value="Làm mới" style="float: left;display:none" /> 
