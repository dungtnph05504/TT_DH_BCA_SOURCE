...
<c:url var="resetPage" value="/servlet/decisionController/updateDecisionManager" />
...
function ClearDDL(){
  ...
  $('#add_btn').click(function() {
	  if(checkValid()){
		  ...
		  stringUrl = $('#decisionManagerForm').serialize(); /* + "&docString="+ encodeURIComponent(base64ImageContent) */;

		  $.post('<c:url value="/servlet/decisionController/updateDecision" />',stringUrl,
		             function(data){
			   if(data=='success'){
			   	   $("#savedialog-confirm").dialog('option', 'title', 'Thông báo');
			       $("#savedialog-confirm").html('Đã sửa thành công Quyết định :  '+$("#decisionNumber").val());
			       ...
			       $("#faildialog-confirm").dialog( 'open' );
				       ...
				       $("#faildialog-confirm").dialog('option', 'title', 'Thông báo');
				       $("#faildialog-confirm").html('Không thể sửa Quyết định:  '+$("#decisionNumber").val());
				       $("#faildialog-confirm").dialog( 'open' );
				}
	  		});
	  }
  ...
  function IsNumeric(sText) {
  	...
  	function openfile(f){
        ...
        window.open("data:application/pdf," + encodeURI(f)); */
        var pdfWindow = window.open("")
        pdfWindow.document.write("<iframe width='100%' height='100%' src='data:application/pdf;base64, " + encodeURI(f)+"'></iframe>")
  	}
		...
		<div class="container">
			<div class="row">
				<div class="roundedBorder ov_hidden">
					<div class="new-heading-2">CẬP NHẬT THÔNG TIN QUYẾT ĐỊNH</div>
					&nbsp;
					<div id="update_msg" style="display: none;">
						<!-- <img src="images/info1.jpg" width="30" height="30"> -->&nbsp;
						<span style="font-size: 14px; color: #0000FF">Quyết định đã được lưu thành công
					</div>
					...
					<form:form modelAttribute="decisionManagerForm"
						...
						name="decisionManagerForm" class="inline">
						<div class="col-sm-6">
							<div class="form-group">
								...
								<form:input class="form-control" type="text"
									path="decisionNumber" id="decisionNumber" readonly="true"/>
								<!--  <input type="text" class="form-control" id="usr">-->
							</div>
							<div class="form-group">
								<label for="competentAuthorities">Chọn cơ quan có thẩm quyến:</label>
							...
							<div class="form-group">
								...
								<form:select style="width: 100%;" path="signer" id="signer"
									...
									<form:option value="">-- SELECT -- </form:option>
									<form:options items="${codeSigner}" itemLabel="nameSigner"
										itemValue="codeSigner"></form:options>
								</form:select>
							</div>
							<div class="form-group">
								<label for="competentAuthoritiesEng">Cơ quan có thẩm
									quyến (bằng Tiếng anh):</label>
								<form:input class="form-control" type="text"
									path="competentAuthoritiesEng" id="competentAuthoritiesEng" />
								<!--  <input type="text" class="form-control" id="usr">-->
							</div>
							<div class="form-group">
								<label for="signDate">Ngày ký quyết định<i
									style="color: red">(*)</i>:
								</label>
								<%-- <form:input class="form-control" type="text" path="signDate"
									id="signDate" /> --%>
						...
						<div class="col-sm-6">
							...
							<div class="form-group">
								<label for="transitNationCode">Quốc gia quá cảnh:</label> <select
									...
									<c:set var="transitN" value="${decisionManagerForm.transitNation}" />
									<c:if test="${not empty codeNational}">
										<c:forEach var="entry" items="${codeNational}">
											<c:choose>
												...
												<c:otherwise>
													<c:choose>
														<c:when test="${not fn:contains(transitN, entry.id.codeValue)}">
															<option value="${entry.id.codeValue}">${entry.codeValueDesc}</option>
														</c:when>
														<c:otherwise>
							...
							<div class="form-group">
								...
								<select
									...
									<c:set var="countryP" value="${decisionManagerForm.countryPlan}" />
									<c:if test="${not empty codeNational}">
										<c:forEach var="entry" items="${codeNational}">
											<c:choose>
												<c:when test="${empty countryP}">
													<option value="${entry.id.codeValue}">${entry.codeValueDesc}</option>
												</c:when>
												<c:otherwise>
												 	<c:choose>
														<c:when test="${not fn:contains(countryP, entry.id.codeValue)}">
															<option value="${entry.id.codeValue}">${entry.codeValueDesc}</option>
														</c:when>
														<c:otherwise>
															<option value="${entry.id.codeValue}" selected="true">${entry.codeValueDesc}</option>
							...
							<div class="form-group">
								<label for="fileU">Tải tài liệu công văn:</label> <input
									class="custom-file-input" type="file" name="fileUploadPath"
									id="fileUploadPath"/>
								<c:set var="fileUp" value="${decisionManagerForm.fileUpload}" />
								<c:if test="${not empty fileUp}">
									<a href="#" onclick="openfile('${fileUp}');">Xem nội dung file đã tải trước đó</a>
								</c:if>
							</div>
							<div class="form-group">
