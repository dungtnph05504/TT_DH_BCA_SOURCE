...
<div class="ov_hidden">
						...
						<div class="form-left">
							<div class="new-heading-2">LẬP DANH SÁCH B</div>
							...
							<div class="col-md-12 col-sm-12" style="padding: 0px;">
								<div class="col-md-10 col-sm-10" style="padding: 0px;height: 250px;overflow: auto;" id="div_chitiet">
									<div>
	...
	<div class="waitingWhenDoneWaiting" style="display:block; position: fixed; bottom:-5px; border-radius: 8px; border: 2px solid; background-color: rgb(255, 255, 255); z-index: +1000;border-color: #2a7044 #2a7044 transparent #2a7044;">
		<div style="margin: 10px"> 
			<button type="button"  onclick="showDocument();" data-toggle="modal" data-target="#taiLieuDK" name="approve" class="btn btn-success">
				<span class="glyphicon glyphicon-file btn-boot" style="margin-right: 2px;"></span ><span class="btn-boot"> Tài liệu đính kèm</span>	
			</button>
			<button type="button"  onclick="printReciept();" id="btn_Print" name="approve" class="btn btn-success">
				<span class="glyphicon glyphicon-print btn-boot" style="margin-right: 2px;"></span ><span class="btn-boot"> In đề xuất</span>	
			</button>
		</div>
	</div>	
	</div>
...
<script type="text/javascript">
	...
	function historyEventDataSmark_clicked(itemTriggered){	 
		...
		/* function printReciept() {
			...
			var yourArray = {};
			var url = '${getInsertRecUrl}/' + danhSachB + '/' + $('#lanhDaoKy').val() + '/' + code + '/' + stt;
			$('#ajax-load-qa').css('display', 'block');
			...
			$.ajax({
				...
				success : function(data) {
					...
					if(data != ''){
						$('#idDeXuat').html(data);
						if(danhSachB == ''){
							$.notify("Lập danh sách B thành công", {
								globalPosition: 'bottom left',
								className: 'success',
							});
							return; 
		...
		function printRecieptC(){
			...
			//var url = '${getInsertRecUrl}/' + danhSachB + "/" + $('#lanhDaoKy').val() + "/N";
			var url = '${getInsertRecUrl}/' + danhSachB + '/' + $('#lanhDaoKy').val() + '/' + code + '/' + stt + '/N/' + noiTra;
			$('#ajax-load-qa').css('display', 'block');
			$.ajax({
				url : url,
				type: "POST",
		...
		function printReciept() {
			...
			if(!testD && !testC && !testK){
				$.notify("Chưa có hồ sơ nào được chọn loại đề xuất!", {
					globalPosition: 'bottom left',
					className: 'warn',
				});
				return;
			...
			//var url = '${getInsertRecUrl}/' + danhSachB + "/" + $('#lanhDaoKy').val() + "/Y";
			var url = '${getInsertRecUrl}/' + danhSachB + '/' + $('#lanhDaoKy').val() + '/' + code + '/' + stt + '/Y/' + noiTra;
			$('#ajax-load-qa').css('display', 'block');
			$.ajax({
				url : url,
				type: "POST",
	...
	$(function() {
	    ...
	    $('#tbDanhSachHS tbody').on('click', 'tr', function(){
	        ...
	        var styleB =  $('#styleList').find(":selected").val();
				...
				success: function(data) {
					...
					if(data.length > 0){
			        	...
			        	for(var i = 0; i < data.length; i++){
			        		...
			        		$('#div_chitiet .dataTables_paginate ').css("display", 'block');
			        		$('#div_chitiet .dataTables_info').css("display", 'block');
			        		$('#tbChiTietDS tbody tr').show();
			        	}
			        	/*-------------------------------------------------*/
			        	/*Thêm số lượng hồ sơ*/
			        	...
			        	/*-------------------------------------------------*/
			        	$("#tbChiTietDS tbody tr:nth-child(1)").addClass("back-gr");
