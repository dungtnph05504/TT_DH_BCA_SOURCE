...
<form:form id="loginForm" modelAttribute="userModel" action="${actionUrl}" method="POST" cssClass="inline">
	...
	<section id="content">
        <h1>Đăng xuất</h1>
        <div>
            <div>Cảm ơn bạn đã sử dụng Hệ thống EPP</div>
        </div>
        <div>
            <div>&nbsp;</div>
        </div>
        <c:if test="${not empty userId and not empty workstationID}">
            <div>
                <div align="center">Đăng xuất thành công cho ${userId} trên ${workstationID}</div>
            </div>
        </c:if>
        <div>
            <div>&nbsp;</div>
...
<div class="button">
        <div>
            <input class="btn btn-primary" type="button" value="Đăng nhập" onclick="login(this.form);" /> &nbsp;
            <input class="btn btn-primary" type="button" value="Đóng" onclick="window.close();" />
        </div>
        </div>
    </section>
	</div>
