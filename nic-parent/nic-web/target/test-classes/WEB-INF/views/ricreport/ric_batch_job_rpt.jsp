...
<c:url var="ricBatchJobResultUrl"
	value="/servlet/batchJobReport/showTabluarResult" />
<c:url var="ricBatchJobPdfResultUrl"
	value="/servlet/batchJobReport/showBatchJobPdf" />
<style>
#inner_content {
	border: thin;
	float: left;
...
<form:form id="ricBatchJobRptForm"
	modelAttribute="batchJbRprtSrchCriteria" method="POST"
	...
	<div id="inner_content">
		<table>
			...
			<tr>
				<td class="subjectsmall" align="left" width="100" height="25">Batch
					Job Run Start Date</td>
				...
				<td class='description_slimplain' align="left" width="250"
					height="25"><form:input type="text" readonly="readonly"
						path="batchJobStartDate" id="startDt" value="${batchJobStartDate}" /></td>
			</tr>
			<tr>
				<td class="subjectsmall" align="left" width="100" height="25">Batch
					Job Run End Date</td>
				...
				<td class='description_slimplain' align="left" width="250"
					height="25"><form:input type="text" readonly="readonly"
						path="batchJobEndDate" id="endDt" value="${batchJobEndDate}" /></td>
					...
					<table width="50%">
						...
						<tr>
							<td><select id="rptcols" multiple="multiple"
								name="rptColumns[]" style="width: 150px;; height: 100px">
							</select></td>
							<td>
								<table>
									<tr>
							...
							<td><form:select id="selRptcols" multiple="multiple"
									...
									<option value="triggeredBy">Triggered By</option>
								</form:select></td>
						</tr>
					</table>
				</td>
			...
			<tr>
				<td class="subjectsmall" height="50" width="300">Job Id</td>
				<td align="left" width="2" height="25">:</td>
				<td class='description_slimplain' align="left" width="100"
					height="25"><form:select id="jobId" path="jobId" width="175">
						<form:option value="ALL" label="ALL" />
						<form:options items="${jobIdList}" id="jobIdList" />
					</form:select></td>
			</tr>
			<tr>
				<td class="subjectsmall" height="50" width="300">Job Status</td>
				<td align="left" width="2" height="25">:</td>
				<td class='description_slimplain' align="left" width="100"
					height="25"><form:select id="jobStatus" path="jobStatus"
			...
			<tr>
				...
				<td class='description_slimplain' align="left" width="100"
					...
					</form:select><%-- <label class="subjectsmall" height="50"
					for="ricOfficId">${batchJbRprtSrchCriteria.site}</label> --%></td>
				<td></td>
			</tr>
			<tr>
				...
				<td align="left" width="2" height="25">:</td>
				<td class='description_slimplain' align="left" width="100"
					height="25"><form:select path="reportFormat" id="rptFrmt">
	...
	<script type="text/javascript">
					...
					.click(
							function() {
								if ($("#rptFrmt").val() == 'pdf') {
									...
									var orig = $("#ricBatchJobRptForm")
											.serialize();
									var withoutEmpties = orig.replace(
											/[^&]+=\.?(?:&|$)/g, '');
									var test = "${ricBatchJobPdfResultUrl}"
											+ '?' + withoutEmpties;
									document.forms["ricBatchJobRptForm"].action = test;
									document.forms["ricBatchJobRptForm"].method = "POST";
									window
											.open(test, "popup",
		...
		function doSubmit(form) {
			...
			$("#showHtmlRpt").empty();
			$("#showHtmlRpt").append($("<table>").attr("id", "batchJbRptflexme"));
			$("#showHtmlRpt").show();
			$("#batchJbRptflexme").flexigrid({
				url : test,
				dataType : 'json',
				colModel : columns,
				sortname : "batchId",
			...
			// $('#batchJbRptflexme').flexAddData(data); 
		}
	</script>
</form:form>
<input type="hidden" id="currentURL" name="currentURL"
	value="${ricBatchJobResultUrl}" />
<%
	} catch (Exception e) {
		e.printStackTrace();
	}
