...
<c:url var="deleteAirportUrl" value="/servlet/adminImmi/deleteAirport" />
<c:url var="editAirportUrl" value="/servlet/adminImmi/editAirport" />
<c:url var="saveChangeAirUrl" value="/servlet/adminImmi/saveChangeAirport" />
<c:url var="showAirportUrl" value="/servlet/adminImmi/showAirport" />
<c:url var="saveAirportUrl" value="/servlet/adminImmi/saveAirport" />
...
<div class="content-item-title">
    <div class="oplog-title__txt">Quản lý dữ liệu sân bay</div>
</div>
...
<form:form modelAttribute="formData" name="formData" autocomplete="off"> 
		    ...
		    <div class="content-item-information">
		     	<fieldset>
		     		...
		     		<div class="form-group mgin-top-10">
		     			<div class="col-sm-4">
		     				<label class="control-lable text-right col-sm-4">Mã sân bay</label>
		     				<div class="col-sm-8">
			...
			<fieldset>
				<legend>Danh sách các sân bay</legend>
							...
							<div style="height: 400px; overflow: auto;">
								  ...
								  <thead>
								    <tr>								     
								      ...
								      <th class="th-sm">Mã sân bay
								
								      </th>
								      <th class="th-sm">Tên sân bay
								   ...
								   <tbody>
								  	<c:if test="${not empty aportList}">
									 
									    <c:forEach items="${aportList}" var="item">
										    <tr>
	  ...
	  <div class="modal-dialog modal-dialog-centered" role="document" style="width: 300px;">
	    <div class="modal-content">
	      ...
	      <div class="modal-footer" style="padding: 5px;">
	       		...
	       		<button type="button" onclick="deleteForm();" class="btn btn-success" style="float: right;margin-right: 10px;">
...
<div id="dialog-confirm"></div>
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			var mesSave = '${mesg}';
			if(mesSave == '1'){
				$.notify('Lưu thông tin sân bay thành công.', {
					globalPosition: 'bottom left',
					className: 'success',
				});
			}else if(mesSave == '0'){
				$.notify('Lưu thông tin sân bay không thành công', {
					globalPosition: 'bottom left',
					className: 'warn',
				});
			}else if(mesSave == '-1'){
				$.notify('Mã sân bay đã tồn tại. Vui lòng thử lại sau', {
					globalPosition: 'bottom left',
					className: 'warn',
				});
			}
			...
			var mesDel = '${result}';
			if(mesDel == '1'){
				$.notify('Xóa thông tin sân bay thành công.', {
					globalPosition: 'bottom left',
					className: 'success',
				});
			}else if(mesDel == '0'){
				$.notify('Xóa thông tin sân bay không thành công', {
					globalPosition: 'bottom left',
					className: 'warn',
				});
			}
	...
	function saveChangeInv(){
		...
		if(codeAir.trim() == ''){
			$.notify('Mã sân bay không được phép bỏ trống.', {
				globalPosition: 'bottom left',
				className: 'warn',
			});
			return;
		...
		if(nameAir.trim() == ''){
			$.notify('Tên sân bay không được phép bỏ trống.', {
				globalPosition: 'bottom left',
				className: 'warn',
			});
			return;
