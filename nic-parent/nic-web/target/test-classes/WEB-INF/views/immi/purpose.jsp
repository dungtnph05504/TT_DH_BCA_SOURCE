...
<c:url var="deletePurposeUrl" value="/servlet/adminImmi/deletePurpose" />
<c:url var="editPurposeUrl" value="/servlet/adminImmi/editPurpose" />
<c:url var="saveChangePurposeUrl" value="/servlet/adminImmi/saveChangePurpose" />
<c:url var="showPurposeUrl" value="/servlet/adminImmi/showPurpose" />
<c:url var="savePurposeUrl" value="/servlet/adminImmi/savePurpose" />
...
<div class="content-item-title">
    <div class="oplog-title__txt">Quản lý mục đích XNC</div>
</div>
...
<form:form modelAttribute="formData" name="formData" autocomplete="off"> 
		    ...
		    <div class="content-item-information">
		     	<fieldset>
		     		...
		     		<div class="form-group mgin-top-10">
		     			...
		     			<div class="col-sm-4">
		     				<label class="control-lable text-right col-sm-4">Mã mục đích</label>
		     				<div class="col-sm-8">
		     					<form:input type="text" class="width-100" path="packageCode"  />
		     				</div>
		     			</div>
		     			<div class="col-sm-4">
			...
			<fieldset>
				<legend>Danh sách mục đích</legend>
							...
							<div style="height: 400px; overflow: auto;">
								  ...
								  <thead>
								    <tr>								     
								      ...
								      <th class="th-sm">Mã mục đích
								
								      </th>							
								      <th class="th-sm">Tên mục đích
								   ...
								   <tbody>
								  	<c:if test="${not empty purposeList}">
									 
									    <c:forEach items="${purposeList}" var="item">
										    <tr>
								  ...
								  <td colspan="6" style="height: 362px">
...
<div id="dialog-confirm"></div>
	<script type="text/javascript">
		...
		window.pagObj = $('#pagination').twbsPagination({
				...
				onPageClick: function (event, page) {
					if (currentPage != page) {
						$('#pageNo').val(page);
						document.forms["formData"].action = '${showPurposeUrl}';  
	...
	function saveChangeInv(){
		...
		function deleteForm(){
			document.forms["formData"].action = '${deletePurposeUrl}/' + idAL;
			document.forms["formData"].submit();
		}
		
		function doApplyFilter() {
			document.forms["formData"].action = '<c:url value="/servlet/adminImmi/showPurpose" />';
			document.forms["formData"].submit();
		}
	
		function editAirline(value, tp) {
			var urlLink = '${editPurposeUrl}';
			$('#divChiTiet').html('');		
			$.ajax({
				type : "POST",
				url : urlLink,
