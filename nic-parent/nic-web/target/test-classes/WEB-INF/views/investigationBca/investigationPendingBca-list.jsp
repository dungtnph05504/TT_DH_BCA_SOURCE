					...
					<div class="ov_hidden"> 
						<!--<div id="heading">
							...
							<div id="heading_left" style="font-weight: bold; background:gray; cursor:pointer;text-decoration:none;color:#fff" align="justify">
								<a style="color:#fff;text-decoration:none" onclick=jobInvestigationConfirmedBcaFunction();>Danh sách Đã xác minh từ BCA </a>
							</div>
						</div>-->
						<div class="new-heading-2 col-md-4">DANH SÁCH CHỜ XÁC MINH BCA</div>
						<div class="new-heading-3 col-md-4"><a onclick=jobInvestigationLoadBcaFunction();>DANH SÁCH GỬI BCA</a></div>
						<div class="new-heading-3 col-md-4"><a onclick=jobInvestigationConfirmedBcaFunction();>DANH SÁCH ĐÃ XÁC MINH BCA</a></div>
						...
						<div style="margin: 2px">
							<div class="cla-form-bca">
								...
								<div class="col-md-4 col-sm-4">
									...
									<div class="col-md-7 col-sm-7 cls-mg-bot">
										<form:select path="validateInfoBca" style="width: 100%;">
											<form:option value="0">Không</form:option>
											<form:option value="1">Đã gửi</form:option>
											<form:option value="2">Đã có kết quả từ BCA</form:option>
										</form:select>
									</div>
									<div class="col-md-12">
										<div style="margin-bottom: 10px;margin-top: 10px;">
											...
											<!--<input type="button" class="btn_small btn-primary btn-search"
												value="Tìm kiếm" style="" onclick="doApplyFilter();" />-->
										</div>
									</div>
									 ...
									 <!-- <div style="vertical-align: text-top; display: inline-block; width: 20%; float: left">
										<div
											style="border: solid 1px gray; margin: 2px; border-radius: 4px; min-height: 50px;">
											<div style="margin: 46px 20px">
							...
							<div>
								  ...
								  <tbody>
								    <c:forEach items="${jobList}" var="item">
									    <tr>
									      <td><c:url var="jobUrl" value="/servlet/investigationBca/startDetailInvestigationP/${item.uploadJobId}" />
									      ...
									      <td><c:choose>
													<c:when test="${item.validateInfoBca_des != 'Không'}">
									                </c:when>
													<c:otherwise>
									                      <form:checkbox path="selectedJobIds" value="${item.transactionId}"
													cssCslas="${item.currentlyAssignedToAnInvestigationOfficer}" />										
									                    </c:otherwise>
											  </c:choose></td>	
									    </tr>
								    </c:forEach>
					...
					<br />
						...
						<c:if test="${not empty jobList}">
							<display:table cellspacing="1" cellpadding="0" id="row"
								...
								requestURI="/servlet/investigationBca/investigationPendingBcaList">
								...
								<display:column title="Chọn" sortable="false">
									<c:choose>
										<c:when test="${row.validateInfoBca_des != 'Không'}">
						                </c:when>
										<c:otherwise>
						                      <form:checkbox path="selectedJobIds" value="${row.transactionId}"
										cssCslas="${row.currentlyAssignedToAnInvestigationOfficer}" />										
						                    </c:otherwise>
									</c:choose>
								</display:column>
							</display:table>
...
<script type="text/javascript">
	...
	window.pagObj = $('#pagination').twbsPagination({
				...
				onPageClick: function (event, page) {
					if (currentPage != page) {
						$('#pageNo').val(page);
						document.forms["formData"].action = '${JobInvestigationPendingBcaUrl}';  
