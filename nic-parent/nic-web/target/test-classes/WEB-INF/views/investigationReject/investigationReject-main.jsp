...
<c:url var="prodUrl" value="/servlet/investigation/startInvestigationConfirm" />
<c:url var="newUrl" value="/servlet/investigation/newInvestigationConfirm" />
...
<!--   </div> -->
		...
		<c:if test="${not empty jobList}">
			<display:table cellspacing="1" cellpadding="0" id="row" sort="external"  partialList="true" size="${totalRecords}"
				...
				requestURI="/servlet/investigation/getInvestigationConfirm"><!-- TRUNG SỬA PHÂN TRANG -->
				...
				<c:url var="jobUrl" value="/servlet/investigation/investigationConfirm/${row.uploadJobId}" />
...
<div id="dialog-confirm"></div>
	<script type="text/javascript">
		function doSubmitSave(form) {
			form.action = '<c:url value="/servlet/investigation/searchInvestigationConfirm" />';//TRUNG SỬA GỌI HÀM BỘ SEARCH
			form.submit();
		}
	</script>
	<script type="text/javascript">
