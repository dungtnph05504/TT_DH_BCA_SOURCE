...
<c:url var="JobErrorPersoUrl" value="/servlet/investigationConfirm/errorPersoList" />
<c:url var="syncPassport" value="/servlet/investigationConfirm/syncPassport" />
	...
	<div class="form-desing">
		<div>
			<div>
				<div class="row">
					<div class="ov_hidden">
						<div class="new-heading-2">DANH SÁCH IN THÀNH CÔNG</div>
							...
							<div>
								  ...
								  <thead>
								    <tr>
								      ...
								      <th class="th-sm" style="min-width: 120px;">Ưu tiên
								      ...
								      <th class="th-sm" style="min-width: 100px;">Số hộ chiếu
								      </th>
								      <th class="th-sm" style="max-width: 100px;">Số biên nhận
								      </th>
								      <th class="th-sm" style="max-width: 100px;">Mã gói
								      </th>
								    </tr>
								  </thead>
								  <tbody>
					...
					<br />
						...
						<c:if test="${not empty jobList}">
							<display:table cellspacing="1" cellpadding="0" id="row"
								...
								requestURI="/servlet/investigationConfirm/successPersoList">								
								...
								<display:column property="recieptNo" sortable="false"
									title="Số biên nhận" maxLength="30" />
								<display:column property="packageId" sortable="false"
									title="Mã gói" maxLength="30" />
								<display:column title="Chọn" sortable="false">
									<c:if test="${row.syncNic}">
...
<!-- 	<div class="waitingWhenDoneWaiting" style="display:block; position: fixed; bottom:5px; right:10px; border-radius: 5px; border: 1px solid gray; background-color: rgb(255, 255, 255); z-index: +1000">
			<div style="margin: 10px"> 
				<input type="button" class="btn btn-primary"              onclick="requestApproveSync();" name="approveSync"  value="Đồng bộ sang Cục lãnh sự" /> 
				&nbsp; 
			</div>
	</div> -->
...
window.pagObj = $('#pagination').twbsPagination({
			...
			onPageClick: function (event, page) {
				if (currentPage != page) {
					$('#pageNo').val(page);
					document.forms["formData"].action = '${jobStatusPPUrl}';  
...
$('#createDate').datepicker().datepicker('setDate', "");
	function doApplyFilter() {
		document.forms["formData"].action = '<c:url value="/servlet/investigationConfirm/applyFilterSuccessPerso" />';
		document.forms["formData"].submit();
	}
	
	function requestApproveSync() { 
		if(confirm("Có chăc chắn muốn thực hiện hành động này?"))
		   {
		 	  	document.forms["formData"].action = '${syncPassport}';  
			   	document.forms["formData"].submit();  
	...
	function jobErrorPersoFunction() {
		document.forms["investigationMenuForm"].action= '${JobErrorPersoUrl}';
