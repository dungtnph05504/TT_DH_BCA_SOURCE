...
<form:form modelAttribute="formData" name="formData">
	<div class="form-desing">
		<div>
			<div>
				<div class="row">
					<div class="ov_hidden">
						<div class="new-heading-2">DANH SÁCH MẤT HỎNG</div>
							...
							<div>
								  ...
								  <thead>
								    <tr>
								      ...
								      <th class="th-sm" style="min-width: 120px;">Loại hộ chiêu
								
								      </th>
								      <th class="th-sm" style="min-width: 120px;">Loại giao dịch	
								      </th>
								      <th class="th-sm" style="max-width: 120px;">Trạng thái
								      </th>
								       <th class="th-sm" style="max-width: 100px;">Lý do
								      </th>
								    </tr>
								  </thead>
								  <tbody>
								    <c:forEach items="${jobList}" var="item">
									    <tr>
									      <td><c:url var="jobUrl" value="/servlet/investigationLost/startDetailInvestigation/${item.uploadJobId}" />
									      ...
									      <td>${item.jobType}</td>
									      <td><c:choose>
													<c:when test="${item.investigationStatus == '02'}">
									                        Đã chuyển
									                </c:when>
													<c:otherwise>
										  ...
										  <td>
										  		<c:url var="noteInvestigationN" value="${item.noteInvestigation}" />
					...
					<br />
						...
						<c:if test="${not empty jobList}">
							<display:table cellspacing="1" cellpadding="0" id="row"
								...
								requestURI="/servlet/investigationLost/investigationLostList">
								...
								<display:column title="ID" sortProperty="uploadJobId"
									sortable="false">
									<c:url var="jobUrl" value="/servlet/investigationLost/startDetailInvestigation/${row.uploadJobId}" />
								...
								<display:column sortable="false" title="Trạng thái chuyển sang BCA">
									<c:choose>
										<c:when test="${row.investigationStatus == '02'}">
						                        Đã chuyển
						                </c:when>
										<c:otherwise>
								...
								<display:column title="Lý do" sortProperty="noteInvestigation" sortable="false">
	...
	<div id="dialog-reject-getRemarks" title="Tạo danh sách bàn giao" style="display: none;"> 
			<div class="form-group">		
				...
				<br />
				<label for="nameHandover">Nội dung danh sách bàn giao</label>
				<input class="form-control" id="nameHandover" name="nameHandover" />
			
			</div> 
	</div> 

	<c:url var="createHandover" value="/servlet/investigationLost/createHandover" />
<%
	/* ******************************************************************************************************************** */
%>
<%
...
//$('.dataTables_length').addClass('bs-select');
	...
	window.pagObj = $('#pagination').twbsPagination({
			...
			onPageClick: function (event, page) {
				if (currentPage != page) {
					$('#pageNo').val(page);
					document.forms["formData"].action = '${listLostUrl}';  
	...
	function doApplyFilter() {
		document.forms["formData"].action = '<c:url value="/servlet/investigationLost/applyFilter" />';
		document.forms["formData"].submit();
	}
