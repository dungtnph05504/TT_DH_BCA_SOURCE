...
<c:url var="deleteAirportUrl" value="/servlet/adminImmi/deleteFlightRouter" />
<c:url var="editAirportUrl" value="/servlet/adminImmi/editFlightRouter" />
<c:url var="saveChangeAirUrl" value="/servlet/adminImmi/saveChangeFlightRouter" />
<c:url var="showAirportUrl" value="/servlet/adminImmi/showFlightRouter" />
<c:url var="saveAirportUrl" value="/servlet/adminImmi/saveFlightRouter" />
...
<div class="content-item-title">
    <div class="oplog-title__txt">Quản lý dữ liệu đường bay</div>
</div>
...
<form:form modelAttribute="formData" name="formData" autocomplete="off"> 
		    ...
		    <div class="content-item-information">
		     	<fieldset>
		     		...
		     		<div class="form-group mgin-top-10">
		     			<div class="col-sm-4">
		     				<label class="control-lable text-right col-sm-4">Mã đường bay</label>
		     				<div class="col-sm-8">
		     					<form:input type="text" class="width-100" path="searchTransactionId" />
		     				</div>
		     			</div>
		     			<div class="col-sm-4">
		     				<label class="control-lable text-right col-sm-4">Sân bay đi</label>
		     				<div class="col-sm-8">
		     			...
		     			<div class="col-sm-4">
		     				<label class="control-lable text-right col-sm-4">Sân bay đến</label>
		     				<div class="col-sm-8">
			...
			<fieldset>
				<legend>Danh sách các đường bay</legend>
							...
							<div style="height: 400px; overflow: auto;">
								  ...
								  <thead>
								    <tr>								     
								      ...
								      <th class="th-sm">Mã đường bay
								
								      </th>							
								      <th class="th-sm">Sân bay đi
								
								      </th>
								      <th class="th-sm">Sân bay đến
								
								      </th>
								      <th class="th-sm">Tên đường bay
								  	...
								  	<c:if test="${not empty flightList}">
									    ...
									    <c:forEach items="${flightList}" var="item">
										    <tr>
										      ...
										      <td>${item.formPlace}</td>										      
										      <td>${item.toPlace}</td>
										      <td>${item.name}</td>
										      <td>${item.note}</td>	
										      <td align="center">
										      	 <a href="#" onclick="editAirline('${item.flightRouterCode}');" >
	...
	<div class="modal fade" id="idChiTiet" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true" >
	  <div class="modal-dialog modal-dialog-centered" role="document" style="width: 900px;">
	    <div class="modal-content">
	      <div class="modal-header">
	        <h5 class="modal-title" id="exampleModalLongTitle" style="display: inline-block;">NHẬP THÔNG TIN ĐƯỜNG BAY</h5>
	        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
	          		<img style="width: 25px;height: 25px;" src="<c:url value='/resources/images/dongform.png' />" title="Đóng" />
	        </button>
	      </div>
	...
	<div class="waitingWhenDoneWaiting" style="display:block; position: fixed; bottom:-5px; border-radius: 8px; border: 2px solid; background-color: rgb(255, 255, 255); z-index: +1000;border-color: #2a7044 #2a7044 transparent #2a7044;">
		<div style="margin: 10px"> 
			<button type="button"  onclick="saveAirline();" name="approve" class="btn btn-success">
				<span class="glyphicon glyphicon-plus"></span> Thêm mới đường bay
			</button>
		</div>
	</div>	
	</div>
<div id="dialog-confirm"></div>
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			var mesSave = '${mesg}';
			if(mesSave == '1'){
				$.notify('Lưu thông tin đường bay thành công.', {
					globalPosition: 'bottom left',
					className: 'success',
				});
			}else if(mesSave == '0'){
				$.notify('Lưu thông tin đường bay không thành công', {
					globalPosition: 'bottom left',
					className: 'warn',
				});
			}else if(mesSave == '-1'){
				$.notify('Mã đường bay đã tồn tại. Vui lòng thử lại sau', {
					globalPosition: 'bottom left',
					className: 'warn',
				});
			}
			...
			var mesDel = '${result}';
			if(mesDel == '1'){
				$.notify('Xóa thông tin đường bay thành công.', {
					globalPosition: 'bottom left',
					className: 'success',
				});
			}else if(mesDel == '0'){
				$.notify('Xóa thông tin đường bay không thành công', {
					globalPosition: 'bottom left',
					className: 'warn',
				});
			}
	...
	function saveChangeInv(){
		...
		var startAir = $('#startAir').val();
		var endAir = $('#endAir').val();
		var nameAir = $('#nameAir').val();
		var descAir = $('#descAir').val();
		var idItem = $('#itemId').val();
		if(codeAir.trim() == ''){
			$.notify('Mã đường bay không được phép bỏ trống.', {
				globalPosition: 'bottom left',
				className: 'warn',
			});
			return;
		...
		if(nameAir.trim() == ''){
			$.notify('Tên đường bay không được phép bỏ trống.', {
				globalPosition: 'bottom left',
				className: 'warn',
			});
			return;
		...
		yourArray['startAir'] = startAir;
		yourArray['endAir'] = endAir;
