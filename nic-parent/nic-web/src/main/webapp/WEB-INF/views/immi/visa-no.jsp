...
<c:url var="deleteVisaUrl" value="/servlet/adminImmi/deleteVisa" />
<c:url var="editVisaUrl" value="/servlet/adminImmi/editVisa" />
<c:url var="saveChangeVisaUrl" value="/servlet/adminImmi/saveChangeVisa" />
<c:url var="showVisaUrl" value="/servlet/adminImmi/showVisa" />
<c:url var="saveVisaUrl" value="/servlet/adminImmi/saveVisa" />
...
<div class="content-item-title">
    <div class="oplog-title__txt">Quản lý dữ liệu thị thực</div>
</div>
...
<form:form modelAttribute="formData" name="formData" autocomplete="off"> 
		    ...
		    <div class="content-item-information">
		     	<fieldset>
		     		...
		     		<div class="form-group mgin-top-10">
		     			<div class="col-sm-4">
		     				<label class="control-lable text-right col-sm-4">Trạng thái</label>
		     				<div class="col-sm-8">
		     					<form:select path="transactionType" class="width-100 sumo-sl" >
		     			...
		     			<div class="col-sm-4">
		     				<label class="control-lable text-right col-sm-4">Quốc tịch</label>
		     				<div class="col-sm-8">
		     					<form:select path="packageCode" class="width-100 sumo-sl" >
			...
			<fieldset>
				<legend>Danh sách thị thực</legend>
							...
							<div style="height: 400px; overflow: auto;">
								  ...
								  <thead>
								    <tr>								     
								      ...
								      <th class="th-sm">Ngày miễn thị thực
								
								      </th>
								      <th class="th-sm">Hiệu lực từ
								
								      </th>
								      <th class="th-sm">Hiệu lực đến
								
								      </th>
								      <th class="th-sm">Được ở tối đa
								
								      </th>
								      <th class="th-sm">Nhập cảnh gần nhất
								  	...
								  	<c:if test="${not empty visaList}">
									    ...
									    <c:forEach items="${visaList}" var="item">
										    <tr>
										      ...
										      <td align="right">${item.freeDay}</td>
										      <td align="center">${item.fromDate}</td>										      
										      <td align="center">${item.toDate}</td>
										      <td align="right">${item.maxDay}</td>
										      <td align="right">${item.lastDay}</td>	
										      <td>${item.status}</td>
...
<div id="dialog-confirm"></div>
	<script type="text/javascript">
		...
		window.pagObj = $('#pagination').twbsPagination({
				...
				onPageClick: function (event, page) {
					if (currentPage != page) {
						$('#pageNo').val(page);
						document.forms["formData"].action = '${showVisaUrl}';  
	...
	function saveChangeInv(){
		...
		function deleteForm(){
			document.forms["formData"].action = '${deleteVisaUrl}/' + idAL;
			document.forms["formData"].submit();
		}
		
		function doApplyFilter() {
			document.forms["formData"].action = '<c:url value="/servlet/adminImmi/showVisa" />';
			document.forms["formData"].submit();
		}
	
		function editAirline(value, tp) {
			var urlLink = '${editVisaUrl}';
			$('#divChiTiet').html('');		
			$.ajax({
				type : "POST",
				url : urlLink,
