...
<c:url var="searchViewDetailsUrl" value="/servlet/dataSyncMonitorController/searchViewDetails" />
<c:url var="showExcelViewDetailResult" value="/servlet/dataSyncMonitorController/generateViewDetailsPDF" />
...
$(function() {
 ...
 $(document).on("click", "#search_view_det_btn",function(){  
	 ...
	 if($("#selectedDate").val()=='' || $("#selectedDate").val()==null){
...
function getSearchViewResults() { 
	$('.modal').show();
	 ...
	 selectedViewDate = $("#selectedDate").val();
...
function generateViewDetailsPDF(){
	$("#selectedDate").val( selectedViewDate);
...
function openFullViewPhotoDialog(transactionId,nin){
	...
	$.ajax({
  	    ...
  	    success : function(data) {				
 		   	 	...
 		   	 	error : function(e){
		   	 		$("#dialog-photoFullView").html('Đã xảy ra lỗi khi hiển thị Ảnh');
	   	    	 	$('.modal').hide();
		   	 	}
	});
}
...
<br /><br />
			...
			<table style="width: 100%; text-align:center; background-color: #E3F1FE" cellspacing="0"
				...
				<tr>
					<td style="border: none; font-weight: bold">Date &nbsp; &nbsp; &nbsp;&nbsp; &nbsp;
						<form:input type="text" id="selectedDate" name="selectedDate" path="selectedDate" cssClass="defaultText" size="20" maxlength="20" readonly="true" />
						&nbsp; &nbsp; &nbsp;&nbsp; &nbsp;
			...
			<form:hidden path="selectedMonth" id="selectedMonth"/>
