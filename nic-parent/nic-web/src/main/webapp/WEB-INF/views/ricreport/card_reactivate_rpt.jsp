...
<c:url var="cardReActTabluarResultUrl"
	value="/servlet/cardStatusReport/showCardReActTabluarResult" /> 
<c:url var="cardReActPdfResultUrl"
value="/servlet/cardStatusReport/showCardReActPdfResult" />
<c:url var="cardReActExcelResultUrl"
value="/servlet/cardStatusReport/showCardReActExcelResult" /> 
<style>
#inner_content {
	border: thin;
	float: left;
...
<form:form id="cardReActivRptForm" modelAttribute="cardStatusRptDto"
	...
	<div id="inner_content">
		<table>
			...
			<tr>
				<td class="subjectsmall" align="left" width="100" height="25">Re
					...
					<table width="50%">
						...
						<tr>
							...
							<td><form:select id="selRptcols"
									...
									<option value="cardIssuDt">Re Activation Date</option>
	...
	<script type="text/javascript">
					...
					.click(
							function() {
								if ($("#rptFrmt").val() == 'pdf') {
									...
									var orig = $("#cardReActivRptForm").serialize();
									var withoutEmpties = orig.replace(
											/[^&]+=\.?(?:&|$)/g, '');
									var test = "${cardReActPdfResultUrl}" + '?'
											+ withoutEmpties;

									document.forms["cardReActivRptForm"].action = test;
									document.forms["cardReActivRptForm"].method = "POST";
									//document.forms["cardReActivRptForm"].onSubmit=window.showModalDialog(test,null, "dialogWidth:1280px;dialogHeight:800px;resizable:yes;addressbar:no;");
									...
									var withoutEmpties = orig.replace(
											/[^&]+=\.?(?:&|$)/g, '');
									var test = "${cardReActExcelResultUrl}" + '?'
											+ withoutEmpties;
									document.forms["cardReActivRptForm"].action = test;
									document.forms["cardReActivRptForm"].method = "POST";
									window
		...
		function doSubmit(form) {
			...
			$("#tranRptflexme").flexigrid({
				...
				sortorder : "asc",
				title : 'CARD RE ACTIVATION REPORT',
				usepager : true,
				useRp : true,
				rp : 10,
...
<input type="hidden" id="currentURL" name="currentURL"
	value="${cardReActTabluarResultUrl}" />
<%
	} catch (Exception e) {
		e.printStackTrace();
	}
