...
<c:url var="ricLostNfoundResultUrl"
	value="/servlet/txnReport/showLostNfoundTabluarResult" />
<c:url var="ricLostNfoundPdfResultUrl"
	value="/servlet/txnReport/showLostNfoundPdfResult" />
<c:url var="ricLostNfoundExcelResultUrl"
	value="/servlet/txnReport/showLostNfoundXlsResult" />
<style>
#inner_content {
	border: thin;
	float: left;
...
<form:form id="ricLostNfoundRptForm" modelAttribute="ricLostNfoundRptDto"
	...
	<div id="inner_content">
		<table>
					...
					<table width="50%">
						...
						<tr>
							...
							<td><form:select id="selRptcols"
									...
									<option value="suspendedDays">No of Days Suspended</option> 
								</form:select></td>
						</tr>
					</table>
				</td>
			...
			<tr>
				<td class="subjectsmall" align="left" width="100" height="25">No
					Of Days Suspended</td>
				<td align="left" width="2" height="25">:</td>
				<td class='description_slimplain' align="left" width="100"
			...
			<tr>
				...
				<td class='description_slimplain' align="left" width="100"
					...
					for="ricOfficId">${siteCodes}</label> <form:hidden id="ricOfficId"
						path="ricOffice" readonly="readonly" value="${siteCodes}" /> --%><form:select id="ricOfficId" path="ricOffice"
						width="200" style="width: 200px">
...
$(function() {  
	...
	$("#genBtnId").click(function(){ 
		 if($("#rptFrmt").val() == 'pdf'){
			...
			var orig = $("#ricLostNfoundRptForm").serialize();
			var withoutEmpties = orig.replace(/[^&]+=\.?(?:&|$)/g, '');
			var test = "${ricLostNfoundPdfResultUrl}" + '?' + withoutEmpties;
			document.forms["ricLostNfoundRptForm"].action = test;  
			document.forms["ricLostNfoundRptForm"].method = "POST";   
			//document.forms["ricLostNfoundRptForm"].onSubmit=window.showModalDialog(test,null, "dialogWidth:1280px;dialogHeight:800px;resizable:yes;addressbar:no;");
			window.open(test, "popup", "location=no,resizable=yes,left=0,height=800px,width=1280px");
			 ...
			 $("#showHtmlRpt").hide(); 
				...
				var orig = $("#ricLostNfoundRptForm").serialize();
				var withoutEmpties = orig.replace(/[^&]+=\.?(?:&|$)/g, '');
				var test = "${ricLostNfoundExcelResultUrl}" + '?' + withoutEmpties; 
				document.forms["ricLostNfoundRptForm"].action = test;  
				document.forms["ricLostNfoundRptForm"].method = "POST";   
	...
	$('#selectAllCol2BtnId').click(function(){
	 ...
	 $("#resetBtn").click(function(){ 
				//EUNIKE 20140116 START
		 		$("#startDt").val('${startDate}'); 
				$("#endDt").val('${endDate}');   
				$("#rptFrmt").val("html");
				//EUNIKE 20140116 END  
				var dataArr = [{'value':'applicationDate','text':'Transaction Date'},
               ...
               {'value':'suspendedDays','text':'No of Days Suspended'} 
									];  
