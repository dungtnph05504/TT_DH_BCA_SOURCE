...
<c:url var="cardColTabluarResultUrl"
	value="/servlet/cardStatusReport/showCardColTabluarResult" /> 
<c:url var="cardColPdfResultUrl"
value="/servlet/cardStatusReport/showCardColPdfResult" />
<c:url var="cardColXlsResultUrl"
value="/servlet/cardStatusReport/showCardColXlsResult" />
<c:url var="cardColExcelResultUrl"
value="/servlet/cardStatusReport/showCardColExcelResult" /> 
<style>
#inner_content {
	border: thin;
	float: left;
...
<form:form id="cardCollRptForm"
	...
	<div id="inner_content">
		<table>
			...
			<tr>
				<td class="subjectsmall" align="left" width="100" height="25">Package Id</td>
				<td align="left" width="2" height="25">:</td>
				<td class='description_slimplain' align="left" width="250" height="25">
					<form:input type="text" size="40" path="pkgId" id="packageId" />
				</td>
			</tr>
			<tr>
						...
						<tr>
							<td><select id="rptcols" multiple="multiple"
								...
								<!--   
										...
										<option value="cardIssuDt">Card Issuance Date</option> 
							...
							<td><form:select id="selRptcols"
									...
									<option value="nin">NIN</option>
										<option value="transactionNo">Transaction No</option>
										<option value="ccnNo">CCN No</option>
										<option value="pkgId">Package Id</option>
			...
			<tr>
				...
				<td class='description_slimplain' align="left" width="100"
					height="25"><form:select id="cardStatus" path="cardStatus">
					<form:option value="" label="ALL" />
						<form:options items="${cardStatusVal}" /></form:select></td>
				<td></td>
			</tr>
			<!--  
			<tr>
				<td class="subjectsmall" height="50" width="300">Issued By</td>
				<td align="left" width="2" height="25">:</td>
				<td class='description_slimplain' align="left" width="100"
					height="25"><form:input type="text" value="" width="15" path="issuedBy"
					id="issuedById"/></td>
				<td></td>
			</tr>
			-->
			...
			$("#resetBtn").click(
					function() {
						...
						var dataArr = [{'value':'nin','text':'NIN'},
						               ...
						               {'value':'pkgId','text':'Package Id'}, 
						               {'value':'pkgSeqNo','text':'Package Sequence No'},  
						               {'value':'firstName','text':'First Name'},
						               {'value':'surName','text':'Surname'},
						               {'value':'surNameAtBirth','text':'Surname at Birth'},
						               {'value':'cardIssuDt','text':'Card Issuance Date'},
									   {'value':'cardStatus','text':'Card Status'}, 
									   {'value':'issuedBy','text':'Issued By'} ]; 
		...
		function doSubmit(form) {  
			...
			var test = $("#currentURL").val() + '?' + withoutEmpties;
			 ...
			 $("#selRptcols option").each(function() {
					...
					if( $(this).text()=='Package Sequence No'){
						jsonObj['width'] = '100';
					}else{
						jsonObj['width'] = '175';
					} 
					...
					columns.push(jsonObj); 
					 ...
					 $("#showHtmlRpt").empty();
						...
						$('#tranRptflexme').flexigrid({ 
							...
							sortorder : 'asc',
							title : 'CARD COLLECTED STATUS REPORT',
							usepager : true,
							useRp : true,
							rp : 10,
