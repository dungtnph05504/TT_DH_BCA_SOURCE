...
<c:url var="ricExprsPrntResultUrl"
	value="/servlet/txnReport/showExprsPrntTabluarResult" />
<c:url var="ricExprsPrntPdfResultUrl"
	value="/servlet/txnReport/showExprsPrntPdfResult" />
<c:url var="ricExprsPrntExcelResultUrl"
	value="/servlet/txnReport/showExprsPrntXlsResult" />
...
<form:form id="ricExprsPrntRptForm" modelAttribute="ricExprsPrntRptDto"
	...
	<div id="inner_content">
		<table>
					...
					<table width="50%">
						...
						<tr>
							...
							<td><form:select id="selRptcols"
									...
									<option value="expressPrinting">Express Printing</option>
									<option value="expressDays">No of Working Days</option>
			...
			<!-- 
			<tr>
				<td class="subjectsmall" height="50" width="300">Transaction Served By</td>
				<td align="left" width="2" height="25">:</td>
				<td class='description_slimplain' align="left" width="100"
					height="25">
						<input type="text" name="txnServedBy" id="txnServed"size="30" />
 				<td></td>
 			</tr>  
 			 -->
...
$(function() {  
	...
	$("#genBtnId").click(function(){ 
		 if($("#rptFrmt").val() == 'pdf'){
			...
			var orig = $("#ricExprsPrntRptForm").serialize();
			var withoutEmpties = orig.replace(/[^&]+=\.?(?:&|$)/g, '');
			var test = "${ricExprsPrntPdfResultUrl}" + '?' + withoutEmpties;
			document.forms["ricExprsPrntRptForm"].action = test;  
			document.forms["ricExprsPrntRptForm"].method = "POST";   
			//document.forms["ricExprsPrntRptForm"].onSubmit=window.showModalDialog(test,null, "dialogWidth:1280px;dialogHeight:800px;resizable:yes;addressbar:no;");
			window.open(test, "popup", "location=no,resizable=yes,left=0,height=800px,width=1280px");
			 ...
			 $("#showHtmlRpt").hide(); 
				...
				var orig = $("#ricExprsPrntRptForm").serialize();
				var withoutEmpties = orig.replace(/[^&]+=\.?(?:&|$)/g, '');
				var test = "${ricExprsPrntExcelResultUrl}" + '?' + withoutEmpties; 
				document.forms["ricExprsPrntRptForm"].action = test;  
				document.forms["ricExprsPrntRptForm"].method = "POST";   
	...
	$('#selectAllCol2BtnId').click(function(){
	 ...
	 $("#resetBtn").click(function(){ 
		 	$("#startDt").val(""); 
               ...
               {'value':'expressPrinting','text':'Express Printing'},
               {'value':'expressDays','text':'No of Working days'},
