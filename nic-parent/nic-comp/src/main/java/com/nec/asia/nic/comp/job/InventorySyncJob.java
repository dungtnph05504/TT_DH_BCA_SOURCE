...
/**
 * The job to sync dispatch info to inventory.
 * 
 * @author khang
 * 
 ...
 */
public class InventorySyncJob extends QuartzJobBean implements StatefulJob {

	private static final Logger logger = LoggerFactory.getLogger(InventorySyncJob.class);
	...
	protected void executeInternal(JobExecutionContext context) throws JobExecutionException {
		logger.info("-----------------InventorySyncJob begins----------------------");
		try {
			List<NicDocumentData> documentDataList = documentDataService.findAllDispatchedDocuments(200);
/*			if (CollectionUtils.isNotEmpty(documentDataList)) {
				...
				for (Map.Entry<String, List<String>> entry : documentMap.entrySet()) {
					logger.debug("----------------- Sync dispatch status for site: "+entry.getKey()+" ----------------------");
			...
			logger.error("[InventorySyncJob] Exception:" + th.getMessage(), th);
			throw new JobExecutionException("Error encountered Update Perso Status job:" + th.getMessage());
		}
		logger.info("----------------------------------------------------------------");
		logger.info("--------- InventorySyncJob Completed --------------------");
		logger.info("----------------------------------------------------------------");
	}
