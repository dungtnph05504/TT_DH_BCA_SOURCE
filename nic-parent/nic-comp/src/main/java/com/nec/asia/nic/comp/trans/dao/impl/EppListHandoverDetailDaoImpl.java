...
public class EppListHandoverDetailDaoImpl extends
	...
	public BaseModelSingle<EppListHandoverDetail> getListPackageByPackageIdAndTranID(
		...
		try {
			DetachedCriteria dCriteria = DetachedCriteria
					.forClass(getPersistentClass());
			if (!(StringUtils.isBlank(packageId) && StringUtils.isBlank(tranID) && StringUtils
					.isBlank(type))) {
				...
				if (list != null && list.size() > 0)
					return new BaseModelSingle<EppListHandoverDetail>(
							list.get(0), true, null);
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public List<EppListHandoverDetail> findAllByTranIdOrType(
		...
		try {
			DetachedCriteria dCriteria = DetachedCriteria
					.forClass(getPersistentClass());
			Criterion test1 = Restrictions.eq("typeList", typeListC);
			Criterion test2 = Restrictions.eq("transactionId", transactionId);
			dCriteria.add(Restrictions.and(test1, test2));
			list = this.findAll(dCriteria);
		} catch (Exception e) {
			e.printStackTrace();
